...
const _PoolV1 = class _PoolV12 extends JettonMinter {
  ...
  async createBurnBody(params) {
    return beginCell().storeUint(DEX_OP_CODES.BURN, 32).storeUint((params == null ? void 0 : params.queryId) ?? 0, 64).storeCoins(BigInt(params.amount)).storeAddress(toAddress(params.responseAddress)).endCell();
  }
  /**
  ...
  async getExpectedOutputs(provider, params) {
    const result = await provider.get("get_expected_outputs", [
      ...
      {
        type: "slice",
        cell: beginCell().storeAddress(toAddress(params.jettonWallet)).endCell()
      }
    ]);
    return {
